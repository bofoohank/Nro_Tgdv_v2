...
namespace NRO_Server.Application.IO
{
    public class ServerUtils
    {
        ...
        public static void WriteTraceLog(string filename, string message)
        {
            try
            {
                var path = $"Logs/Tracelogs/{DatabaseManager.Manager.gI().ServerPort}/{DateTime.Now.ToString("MM-dd-yyyy")}/{filename}.txt";
        ...
        public static void WriteLogBug(string charname, int userid)
        {
            try
            {
                var path = $"Logs/Svlogs/Logbugs.txt";
                new FileInfo(path).Directory.Create();
        ...
        public static void WriteLog(string filename, string message)
        {
            try
            {
                var path = $"Logs/Svlogs/{DatabaseManager.Manager.gI().ServerPort}/{DateTime.Now.ToString("MM-dd-yyyy")}/{filename}.txt";
                new FileInfo(path).Directory.Create();
        ...
        public static void WriteTradeLog(string message, int tongThoiVangGiaoDich)
        {
            try
            {
                var path = $"Logs/Tradelogs/{DatabaseManager.Manager.gI().ServerPort}/{DateTime.Now.ToString("MM-dd-yyyy")}/Trade.txt";
                if (tongThoiVangGiaoDich > 300)
                {
                    path = $"Logs/Tradelogs/{DatabaseManager.Manager.gI().ServerPort}/{DateTime.Now.ToString("MM-dd-yyyy")}/Tradethoivang.txt";
                }
                new FileInfo(path).Directory.Create();
