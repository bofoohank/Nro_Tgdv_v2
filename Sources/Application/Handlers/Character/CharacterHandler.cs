...
namespace NRO_Server.Application.Handlers.Character
{
    public class CharacterHandler : ICharacterHandler
    {
        ...
        public void UpdateEffect(long timeServer)
        {
            ...
            if (Character.InfoBuff.CuongNoProTime < timeServer && Character.InfoBuff.CuongNoPro)
            {
                Character.InfoBuff.CuongNoPro = false;
                IsRemoveBuffEffect = true;
            }
            // Effect cuồng nộ
            ...
            // Effect thuốc hồi trinh
            if (Character.InfoBuff.ThuocHoiTrinhTime < timeServer && Character.InfoBuff.ThuocHoiTrinh)
            {
                Character.InfoBuff.ThuocHoiTrinh = false;
                IsRemoveBuffEffect = true;
            }
            // Effect Bổ huyết
        ...
        private void UpdateFusion(long timeServer)
        {
            ...
            if (disciple is { Status: 4 } && fusion.IsFusion)
            {
                ...
                if (timeServer >= fusion.TimeStart + fusion.TimeUse && (!fusion.IsPorata && !fusion.IsPorata2 && !fusion.IsPorata3 && !fusion.IsPorata4))
                {
        ...
        public void UpdatePhukien()
        {
            ...
            var itemBag = Character.ItemBody[7];
            if (itemBag != null)
            {
        ...
        public int GetParamItem(int id)
        {
            ...
            var itemBag = Character.ItemBody.FirstOrDefault(item => item != null && ItemCache.ItemTemplate(item.Id).Type == 11);
        ...
        public void SetInfoBuff()
        {
            ...
            if (Character.InfoBuff.CuongNoProTime > timeServer)
            {
                var giayConLai = (Character.InfoBuff.CuongNoProTime - timeServer) / 1000;
                if (giayConLai <= 0) giayConLai = 0;
                SendMessage(Service.ItemTime(10716, (int)giayConLai));
            }
            // Effect thuốc hồi trinh
            if (Character.InfoBuff.ThuocHoiTrinhTime > timeServer)
            {
                var giayConLai = (Character.InfoBuff.ThuocHoiTrinhTime - timeServer) / 1000;
                if (giayConLai <= 0) giayConLai = 0;
                SendMessage(Service.ItemTime(13276, (int)giayConLai));
            }
            // Effect Bổ huyết
        ...
        public void SetHpFull()
        {
            ...
            if (Character.InfoChar.Fusion.IsFusion && Character.Disciple != null)
            {
                ...
                // Đệ thường +100%
                else if (Character.Disciple.Type == 2)
                {
                ...
                {
                    var bongTaiPorata3 = GetItemBagById(1108);
                    if (bongTaiPorata3 != null)
                    {
                        var optionCheck = bongTaiPorata3.Options.FirstOrDefault(option => option.Id != 72);
                        if (optionCheck != null && optionCheck.Id == 77)
                        {
                            hp += hp * optionCheck.Param / 100;
                        }
                ...
                {
                    var bongTaiPorata4 = GetItemBagById(1110);
                    if (bongTaiPorata4 != null)
                    {
                        var optionCheck = bongTaiPorata4.Options.FirstOrDefault(option => option.Id != 72);
                        if (optionCheck != null && optionCheck.Id == 77)
                        {
                            hp += hp * optionCheck.Param / 100;
                        }
            ...
            //Thuốc hồi trinh
            if (Character.InfoBuff.ThuocHoiTrinh)
            {
                hp = hp + 1000000;
            }
        ...
        public void SetDamageFull()
        {
			...
			GetListParamItem(233).ForEach(param => damage += param);
            ...
            // Cuồng nộ2
            if (Character.InfoBuff.CuongNoPro)
            {
        ...
        {
            var check = Character.ItemBag.ToList();
        ...
        // THÊM VẬT PHẨM VÀO HÀNH TRANG KHI NHẶT TỪ MAP
        ...
        // THÊM VẬT PHẨM VÀO HÀNH TRANG KHI THÁO TRANG BỊ HOẶC MUA TỪ SHOP
        ...
        public void PlusPoint(IMonster monster, int damage)
        {
			...
			var IdMapKhongTNSM = DataCache.IdMapKhongTNSM.Contains(Character.InfoChar.MapId);
        ...
        public void LeaveFromDead(bool isHeal = false)
        {
            lock (Character)
            {
                if (!isHeal)
                {
                    ...
                    //20tr vàng
					var vangTrongNguoi = Character.InfoChar.Gold;
                    if (vangTrongNguoi < 20000000)
                    {
                        SendMessage(Service.ServerMessage(TextServer.gI().NOT_ENOUGH_GOLD));
                        return;
                    }
                    Character.MineGold(20000000);
                }
                SendMessage(Service.MeLoadInfo(Character));
                        ...
                        case 73:
                            {
                                ...
                                zone.ZoneHandler.SendMessage(Service.ItemMapMePick(itemMap.Id, itemNew.Quantity, "nhặt từ map"));
                                break;
                            }
                        case 78:
                            {
