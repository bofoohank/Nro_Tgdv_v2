...
namespace NRO_Server.Application.Handlers.Character
{
    public class BossHandler : ICharacterHandler
    {
        ...
        private void HandleUseSkill(ICharacter character)
        {
                 ...
                 infoSkill.TaiTaoNangLuong.DelayTTNL > timeServer)
            {
                if (Boss.InfoDelayBoss.TTNL <= timeServer)
                {
                    ...
                    Boss.InfoDelayBoss.TTNL = timeServer + 108000;
                }
                return;
            }
        ...
        private void CombatChatMessage(long timeServer)
        {
			...
			else if (Boss.Type == DataCache.BOSS_CUMBER_TYPE || Boss.Type == DataCache.BOSS_SUPER_CUMBER_TYPE)
			{
				SendBossChat(TextServer.gI().TGDV_BOSS_CHAT[ServerUtils.RandomNumber(TextServer.gI().TGDV_BOSS_CHAT.Count)]);
			}
            else
            {
        ...
        public void LeaveItem(ICharacter character)
        {
            ...
            switch (Boss.Type)
            {
                case 1://super broly
                    {
                        var @char = (NRO_Server.Model.Character.Character)character;
                        if (@char.Disciple == null)
                        {
                            var detu = new Disciple();
                            detu.CreateNewDisciple(@char);
                            detu.Player = character.Player;
                            detu.CharacterHandler.SetUpInfo();

                            @char.Disciple = detu;
                            @char.InfoChar.IsHavePet = true;
                            @char.CharacterHandler.SendMessage(Service.Disciple(1, null));
                            @char.Zone.ZoneHandler.AddDisciple(detu);
                            DiscipleDB.Create(detu);
                            character.CharacterHandler.SendMeMessage(Service.PublicChat(detu.Id,"Hế lô sư phụ"));
                        }
                        break;
                    }
				case 57://Cumber
				{
					...
					if (phanTramRoiDo <= 10)//đồ thần linh
					{
						var item = ItemCache.GetItemDefault(((short)ServerUtils.RandomNumber(DataCache.ListDoThanLinh.Count)));
					...
					else if (phanTramRoiDo < 60) //đá ngục tù
                    {
                        var listDaNgucTu = new List<short>() {1112, 1113, 1114};
						var item = ItemCache.GetItemDefault(listDaNgucTu[ServerUtils.RandomNumber(listDaNgucTu.Count)]);
					...
					else if (phanTramRoiDo < 80) //100tr vàng
                    {
				...
				case 58://Super Cumber
				{
					var phanTramRoiDo = ServerUtils.RandomNumber(0, 100);
					if (phanTramRoiDo < 30)//đồ thần linh
					{
						var item = ItemCache.GetItemDefault(((short)ServerUtils.RandomNumber(DataCache.ListDoThanLinh.Count)));
                        var itemMap = new ItemMap(playerKillId, item);
                        itemMap.X = Boss.InfoChar.X;
					...
					else if (phanTramRoiDo < 100) //đá ngục tù
                    {
                        var listDaNgucTu = new List<short>() {1112, 1113, 1114};
						var item = ItemCache.GetItemDefault(listDaNgucTu[ServerUtils.RandomNumber(listDaNgucTu.Count)]);
						item.Quantity = ServerUtils.RandomNumber(3, 8);
						var itemMap = new ItemMap(playerKillId, item);
                        itemMap.X = Boss.InfoChar.X;
                        itemMap.Y = Boss.InfoChar.Y;
                        Boss.Zone.ZoneHandler.LeaveItemMap(itemMap);
                    }
					else if (phanTramRoiDo < 90) //100tr vàng
                    {
                        var item = ItemCache.GetItemDefault(190);
						item.Quantity = 100000000;
                        var itemMap = new ItemMap(playerKillId, item);
        ...
        private Model.Item.Item RandomItemDrop(short idItem, int start)
        {
            var item = ItemCache.GetItemDefault(idItem);
            var soSao = ServerUtils.RandomNumber(0, start);
            if (soSao > 0)
            {
