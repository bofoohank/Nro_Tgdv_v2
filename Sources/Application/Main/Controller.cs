    ...
    public class Controller : IMessageHandler
    {
        ...
        public async Task OnMessage(Message message)
        {
            try
            {
                ...
                switch (command)
                {
                            ...
                            try
                            {
                                ...
                                if (DataCache.IdMapCustom.Contains(mapId))
                                {
                                    ...
                                    switch (mapId)
                                    {
										...
										case 50:
                                            {
                                                JoinKarin(50, true, true, character.InfoChar.Teleport);
                                                return;
                                            }
                                        case 111:
                                            {
                                    ...
                                    case 2:
                                        {
                                            ...
                                            character.CharacterHandler.SendMessage(Service.PublicChat(disciple.Id,
                                                "Ok, sư phụ một mình con chấp hết"));
                                            disciple.Status = 2;
                                            break;
                                        }
                                    case 3:
                                    ...
                                    case 4:
                                        {
                                            ...
                                            if (character.InfoChar.Fusion.IsFusion)
                                            {
                                                ...
                                                lock (character.InfoChar.Fusion)
                                                {
													...
													character.InfoChar.Fusion.IsPorata3 = false;
													character.InfoChar.Fusion.IsPorata4 = false;
                    ...
                    #region Accept Please Join Clan
                    case -49:
                        {
                            var character = _session?.Player?.Character;
                            if (DatabaseManager.Manager.gI().IsVIPServer)
                    ...
                    case -40:
                        {
                            ...
                            switch (type)
                            {
                                ...
                                #region REMOVE ITEM FROM DISCIPLE
                                case 7:
                                    {
                                        var charReal = (Character)character;
                    ...
                    case 44:
                        {
                            ...
                            if (text.Contains("ten con la "))
                            {
                                var itemRename = character.ItemBag.FindIndex(item => item.Id == 400);
                                if (itemRename < 0)
                                {
                                    character.CharacterHandler.SendMessage(Service.ServerMessage("Đi mua cái thẻ đặt tên đi. có đâu mà đòi đặt"));
                                    return;
                                }
                                var nameDiciple = text.Replace("ten con la ", "");
                                var discipleInZone = character?.Zone.Disciples;
                                var hasDisciple = false;
                                Parallel.ForEach(discipleInZone, disciple =>
                                {
                                    if (disciple.Key == -character.Id)
                                    {
                                        var discipleUpdate = disciple.Value;
                                        character.Zone.ZoneHandler.RemoveDisciple(character.Disciple);
                                        discipleUpdate.Name = nameDiciple;
                                        DiscipleDB.SaveInventory(discipleUpdate);
                                        character.Disciple = discipleUpdate;
                                        character.Zone.ZoneHandler.AddDisciple(discipleUpdate);
                                        hasDisciple = true;
										character.CharacterHandler.SendMessage(Service.ServerMessage("Đổi tên đệ thành công"));
                                        character.CharacterHandler.RemoveItemBag(itemRename, true, "đổi tên để tử");
                                    }
                                });
                                if (!hasDisciple)
                                {
                                    character.CharacterHandler.SendMessage(Service.ServerMessage("Không tìm thấy đồ đệ của bạn ở đây"));
                                }
                                else
                                {
                            ...
                            if (!text.Equals("") && character != null && role == 1)
                            {
                                ...
                                if (text.Contains("cp"))
                                {
                                    ...
                                    {
                                        var idpet = text.Replace("cp", "").Trim();
                                ...
                                else if (text.Contains("cb"))
                                {
                                    if (character.Pet == null)
                                    {
                                        var idBoss = text.Replace("cb", "").Trim();
                                        var boss = int.Parse(idBoss);
                                        var superBroly = new Boss();
                                        superBroly.CreateBoss(boss, character.InfoChar.X, character.InfoChar.Y);
        ...
        private void MessageSubCommand(Message message)
        {
            try
            {
                ...
                switch (command)
                {
                    #region Cộng điểm tiềm năng vào chỉ số
                    case 16:
                        {
                            var type = message.Reader.ReadByte();
                                    ...
                                    case 2:
                                        {
                                            ...
                                            if (character.InfoChar.Task.Id == 30 && character.InfoChar.Task.Index == 0)
                                            {
                                                if (damageNew > 10000)
                                                {
                                                    ...
                                                    character.CharacterHandler.SendMessage(Service.ServerMessage("Ái chà chà mạnh phết rồi đấy, ra quân thôi"));
                                                }
                                            }
                                            break;
                                        }
        ...
        private void NextMap()
        {
            ...
            if (wayPoint.MapNextId is 47 or 48 or 45 or 46 or 50 or 111 && !DataCache.IdMapCustom.Contains(mapOld.Id))
            {
                mapOld.OutZone(character, wayPoint.MapNextId);
                ...
                case 1:
                    {
                        ...
                        switch (character.ShopId)
                        {
							...
							case 5:
							{
								...
								{
									character.CharacterHandler.SendMessage(
                                        Service.DialogMessage(TextMenuNCBT.Gi().TRANG_BI_LOI_PORATA_2));
                                    return Task.CompletedTask;
								}
								...
								{
									character.CharacterHandler.SendMessage(
                                        Service.DialogMessage(TextMenuNCBT.Gi().TRANG_BI_LOI_PORATA_2_FIRST));
                                    return Task.CompletedTask;
								}
                                ...
                                {
                                    character.CharacterHandler.SendMessage(
                                        Service.DialogMessage(TextMenuNCBT.Gi().TRANG_BI_LOI_PORATA_2_SECOND));
                                    return Task.CompletedTask;
                                }
								var soLuongManhVoBongTai = itemManhVoBongTai.Options.FirstOrDefault(opt => opt.Id == 31); //Số lượng bông tai
                                if (soLuongManhVoBongTai == null || soLuongManhVoBongTai.Param < 99)
                                {
                                    character.CharacterHandler.SendMessage(
                                        Service.DialogMessage(TextMenuNCBT.Gi().TRANG_BI_LOI_PORATA_2_SECOND));
                                    return Task.CompletedTask;
								...
								var dataCombinne = NCBT.PercentNangCapPorata2;
                                ...
                                info += $"{ServerUtils.Color("blue")}Cần 99 Mảnh vỡ bông tai";
                                info += $"{ServerUtils.Color(checkGold ? "blue" : "red")}{string.Format(TextServer.gI().NEED_GOLD, ServerUtils.GetMoney(dataCombinne[1]))}";
                                info += $"{ServerUtils.Color(checkDiamond ? "blue" : "red")}{string.Format(TextServer.gI().NEED_DIAMOND, dataCombinne[0])}";
                                info += $"{ServerUtils.Color("red")}Thất bại -49 mảnh vỡ bông tai";
								
								character.CharacterHandler.SendMessage(Service.OpenUiConfirm(78, info,new List<string>() { "Nâng cấp\n" + ServerUtils.GetMoney(dataCombinne[1]) + " vàng\n" + dataCombinne[0] + " ngọc", "Từ chối" },character.InfoChar.Gender));
							...
							case 6:
							{
								...
								{
									character.CharacterHandler.SendMessage(
                                        Service.DialogMessage(TextMenuNCBT.Gi().TRANG_BI_LOI_PORATA_3));
                                    return Task.CompletedTask;
								}
								...
								{
									character.CharacterHandler.SendMessage(
                                        Service.DialogMessage(TextMenuNCBT.Gi().TRANG_BI_LOI_PORATA_3_FIRST));
                                    return Task.CompletedTask;
								}
                                ...
                                {
                                    character.CharacterHandler.SendMessage(
                                        Service.DialogMessage(TextMenuNCBT.Gi().TRANG_BI_LOI_PORATA_3_SECOND));
                                    return Task.CompletedTask;
                                }
								var soLuongManhVoBongTai = itemManhVoBongTai.Options.FirstOrDefault(opt => opt.Id == 31); //Số lượng bông tai
                                if (soLuongManhVoBongTai == null || soLuongManhVoBongTai.Param < 999)
                                {
                                    character.CharacterHandler.SendMessage(
                                        Service.DialogMessage(TextMenuNCBT.Gi().TRANG_BI_LOI_PORATA_3_SECOND));
                                    return Task.CompletedTask;
								...
								var dataCombinne = NCBT.PercentNangCapPorata3;
                                var checkDiamond = dataCombinne[0] < character.AllDiamond();
                                var checkGold = dataCombinne[1] < character.InfoChar.Gold;
								
								var info = $"{ServerUtils.Color("blue")}Bông tai Porata 3";
                                info += $"{ServerUtils.Color("blue")}{string.Format(TextServer.gI().PERCENT_UPGRADE, dataCombinne[2])}%";
                                info += $"{ServerUtils.Color("blue")}Cần 999 Mảnh vỡ bông tai";
                                info += $"{ServerUtils.Color(checkGold ? "blue" : "red")}{string.Format(TextServer.gI().NEED_GOLD, ServerUtils.GetMoney(dataCombinne[1]))}";
                                info += $"{ServerUtils.Color(checkDiamond ? "blue" : "red")}{string.Format(TextServer.gI().NEED_DIAMOND, dataCombinne[0])}";
                                info += $"{ServerUtils.Color("red")}Thất bại -499 mảnh vỡ bông tai";
							...
							case 7:
							{
								...
								{
									character.CharacterHandler.SendMessage(
                                        Service.DialogMessage(TextMenuNCBT.Gi().TRANG_BI_LOI_PORATA_4));
                                    return Task.CompletedTask;
								}
								
								var itemBongTai = character.CharacterHandler.GetItemBagByIndex(arrIndexUi[0]);
								if (itemBongTai == null || itemBongTai.Id != 1108)
								{
									character.CharacterHandler.SendMessage(
                                        Service.DialogMessage(TextMenuNCBT.Gi().TRANG_BI_LOI_PORATA_4_FIRST));
                                    return Task.CompletedTask;
								}
                                ...
                                {
                                    character.CharacterHandler.SendMessage(
                                        Service.DialogMessage(TextMenuNCBT.Gi().TRANG_BI_LOI_PORATA_4_SECOND));
                                    return Task.CompletedTask;
                                }
								var soLuongManhVoBongTai = itemManhVoBongTai.Options.FirstOrDefault(opt => opt.Id == 31); //Số lượng bông tai
								...
								var dataCombinne = NCBT.PercentNangCapPorata4;
                                var checkDiamond = dataCombinne[0] < character.AllDiamond();
                                var checkGold = dataCombinne[1] < character.InfoChar.Gold;
								
								var info = $"{ServerUtils.Color("blue")}Bông tai Porata 4";
                                info += $"{ServerUtils.Color("blue")}{string.Format(TextServer.gI().PERCENT_UPGRADE, dataCombinne[2])}%";
                                info += $"{ServerUtils.Color("blue")}Cần 9999 Mảnh vỡ bông tai";
                                info += $"{ServerUtils.Color(checkGold ? "blue" : "red")}{string.Format(TextServer.gI().NEED_GOLD, ServerUtils.GetMoney(dataCombinne[1]))}";
                                info += $"{ServerUtils.Color(checkDiamond ? "blue" : "red")}{string.Format(TextServer.gI().NEED_DIAMOND, dataCombinne[0])}";
                                info += $"{ServerUtils.Color("red")}Thất bại -4999 mảnh vỡ bông tai";
							...
							#region Mở chỉ số bông tai
							case 8:
							{
								//Kiểm tra vật phẩm
								if (arrIndexUi.Count != 3)
                                {
                                    character.CharacterHandler.SendMessage(Service.DialogMessage(TextMenuNCBT.Gi().UPGRADE_OPTION_PORATA_2_LOI_COUNT));
                                    return Task.CompletedTask;
                                }
                                ...
                                if (itemBongTai == null || itemBongTai.Id != 921)
								{
									character.CharacterHandler.SendMessage(Service.DialogMessage(TextMenuNCBT.Gi().UPGRADE_OPTION_PORATA_2_FIRST));
                                    return Task.CompletedTask;
								}
								...
								{
									character.CharacterHandler.SendMessage(Service.DialogMessage(TextMenuNCBT.Gi().UPGRADE_OPTION_PORATA_2_SECOND));
                                    return Task.CompletedTask;
								}
								...
								{
									character.CharacterHandler.SendMessage(Service.DialogMessage(TextMenuNCBT.Gi().UPGRADE_OPTION_PORATA_2_THIRD));
                                    return Task.CompletedTask;
								}
								
								var dataCombinne = NCBT.PercentMoChiSoPorata2;
                                var checkDiamond = dataCombinne[0] < character.AllDiamond();
                                var checkGold = dataCombinne[1] < character.InfoChar.Gold;
								
								var info = $"{ServerUtils.Color("blue")}{string.Format(TextServer.gI().PERCENT_UPGRADE, dataCombinne[2])}%";
                                info += $"{ServerUtils.Color("blue")}Cần 99 Mảnh hồn bông tai và 1 Đá xanh lam";
                                //info += $"{ServerUtils.Color("blue")}Cần 1 Đá xanh lam";
                                info += $"{ServerUtils.Color(checkGold ? "blue" : "red")}{string.Format(TextServer.gI().NEED_GOLD, ServerUtils.GetMoney(dataCombinne[1]))}";
                                info += $"{ServerUtils.Color(checkDiamond ? "blue" : "red")}{string.Format(TextServer.gI().NEED_DIAMOND, dataCombinne[0])}";
                                info += $"{ServerUtils.Color("green")}+1 Chỉ số ngẫu nhiên:";
								info += $"{ServerUtils.Color("red")}Sức đánh (Min: 7, Max: 20)";
								info += $"{ServerUtils.Color("red")}Hp (Min: 10, Max: 20)";
								info += $"{ServerUtils.Color("red")}Ki (Min: 10, Max: 20)";
								info += $"{ServerUtils.Color("red")}Giáp (Min: 5, Max: 20)";
								info += $"{ServerUtils.Color("red")}Chí mạng (Min: 2, Max: 15)";
								info += $"{ServerUtils.Color("red")}Né đòn (Min: 5, Max: 20)";
							...
							case 9:
							{
								...
								if (arrIndexUi.Count != 3)
                                {
                                    character.CharacterHandler.SendMessage(Service.DialogMessage(TextMenuNCBT.Gi().UPGRADE_OPTION_PORATA_3_LOI_COUNT));
                                    return Task.CompletedTask;
                                }
                                ...
                                if (itemBongTai == null || itemBongTai.Id != 1108)
								{
									character.CharacterHandler.SendMessage(Service.DialogMessage(TextMenuNCBT.Gi().UPGRADE_OPTION_PORATA_3_FIRST));
                                    return Task.CompletedTask;
								}
								
								var itemManhVoBongTai = character.CharacterHandler.GetItemBagByIndex(arrIndexUi[1]);
                                if (itemManhVoBongTai == null || itemManhVoBongTai.Id != 934 || itemManhVoBongTai.Quantity < 199)
								{
									character.CharacterHandler.SendMessage(Service.DialogMessage(TextMenuNCBT.Gi().UPGRADE_OPTION_PORATA_3_SECOND));
                                    return Task.CompletedTask;
								}
								
								var itemDaXanhLam = character.CharacterHandler.GetItemBagByIndex(arrIndexUi[2]);
								if (itemDaXanhLam == null || itemDaXanhLam.Id != 935 || itemDaXanhLam.Quantity < 5)
								{
									character.CharacterHandler.SendMessage(Service.DialogMessage(TextMenuNCBT.Gi().UPGRADE_OPTION_PORATA_3_THIRD));
                                    return Task.CompletedTask;
								}
								
								var dataCombinne = NCBT.PercentMoChiSoPorata3;
                                var checkDiamond = dataCombinne[0] < character.AllDiamond();
                                var checkGold = dataCombinne[1] < character.InfoChar.Gold;
								
								var info = $"{ServerUtils.Color("blue")}{string.Format(TextServer.gI().PERCENT_UPGRADE, dataCombinne[2])}%";
                                info += $"{ServerUtils.Color("blue")}Cần 199 Mảnh hồn bông tai và 5 Đá xanh lam";
                                info += $"{ServerUtils.Color(checkGold ? "blue" : "red")}{string.Format(TextServer.gI().NEED_GOLD, ServerUtils.GetMoney(dataCombinne[1]))}";
                                info += $"{ServerUtils.Color(checkDiamond ? "blue" : "red")}{string.Format(TextServer.gI().NEED_DIAMOND, dataCombinne[0])}";
                                info += $"{ServerUtils.Color("green")}+1 Chỉ số ngẫu nhiên:";
								info += $"{ServerUtils.Color("red")}Sức đánh (Min: 7, Max: 25)";
								info += $"{ServerUtils.Color("red")}Hp (Min: 10, Max: 25)";
								info += $"{ServerUtils.Color("red")}Ki (Min: 10, Max: 25)";
								info += $"{ServerUtils.Color("red")}Giáp (Min: 5, Max: 25)";
								info += $"{ServerUtils.Color("red")}Chí mạng (Min: 2, Max: 20)";
								info += $"{ServerUtils.Color("red")}Né đòn (Min: 5, Max: 25)";
							...
							case 10:
							{
								...
								if (arrIndexUi.Count != 3)
                                {
                                    character.CharacterHandler.SendMessage(Service.DialogMessage(TextMenuNCBT.Gi().UPGRADE_OPTION_PORATA_4_LOI_COUNT));
                                    return Task.CompletedTask;
                                }
								
								var itemBongTai = character.CharacterHandler.GetItemBagByIndex(arrIndexUi[0]);
                                if (itemBongTai == null || itemBongTai.Id != 1110)
								{
									character.CharacterHandler.SendMessage(Service.DialogMessage(TextMenuNCBT.Gi().UPGRADE_OPTION_PORATA_4_FIRST));
                                    return Task.CompletedTask;
								}
								
								var itemManhVoBongTai = character.CharacterHandler.GetItemBagByIndex(arrIndexUi[1]);
                                if (itemManhVoBongTai == null || itemManhVoBongTai.Id != 934 || itemManhVoBongTai.Quantity < 299)
								{
									character.CharacterHandler.SendMessage(Service.DialogMessage(TextMenuNCBT.Gi().UPGRADE_OPTION_PORATA_4_SECOND));
                                    return Task.CompletedTask;
								}
								
								var itemDaXanhLam = character.CharacterHandler.GetItemBagByIndex(arrIndexUi[2]);
								if (itemDaXanhLam == null || itemDaXanhLam.Id != 935 || itemDaXanhLam.Quantity < 10)
								{
									character.CharacterHandler.SendMessage(Service.DialogMessage(TextMenuNCBT.Gi().UPGRADE_OPTION_PORATA_4_THIRD));
                                    return Task.CompletedTask;
								}
								
								var dataCombinne = NCBT.PercentMoChiSoPorata4;
                                var checkDiamond = dataCombinne[0] < character.AllDiamond();
                                var checkGold = dataCombinne[1] < character.InfoChar.Gold;
								
								var info = $"{ServerUtils.Color("blue")}{string.Format(TextServer.gI().PERCENT_UPGRADE, dataCombinne[2])}%";
                                info += $"{ServerUtils.Color("blue")}Cần 299 Mảnh hồn bông tai và 10 Đá xanh lam";
                                //info += $"{ServerUtils.Color("blue")}Cần 1 Đá xanh lam";
                                info += $"{ServerUtils.Color(checkGold ? "blue" : "red")}{string.Format(TextServer.gI().NEED_GOLD, ServerUtils.GetMoney(dataCombinne[1]))}";
                                info += $"{ServerUtils.Color(checkDiamond ? "blue" : "red")}{string.Format(TextServer.gI().NEED_DIAMOND, dataCombinne[0])}";
                                info += $"{ServerUtils.Color("green")}+1 Chỉ số ngẫu nhiên:";
								info += $"{ServerUtils.Color("red")}Sức đánh (Min: 7, Max: 30)";
								info += $"{ServerUtils.Color("red")}Hp (Min: 10, Max: 30)";
								info += $"{ServerUtils.Color("red")}Ki (Min: 10, Max: 30)";
								info += $"{ServerUtils.Color("red")}Giáp (Min: 5, Max: 30)";
								info += $"{ServerUtils.Color("red")}Chí mạng (Min: 2, Max: 25)";
								info += $"{ServerUtils.Color("red")}Né đòn (Min: 5, Max: 30)";
							...
							case 11:
							{
								if (arrIndexUi.Count != 2)
                                {
                                    character.CharacterHandler.SendMessage(Service.DialogMessage(TextKhamDa.Gi().THIEU_NGUYEN_LIEU));
                                    return Task.CompletedTask;
                                }
                                ...
                                if (itemBag == null )
								{
									character.CharacterHandler.SendMessage(Service.DialogMessage(TextKhamDa.Gi().THIEU_NGUYEN_LIEU_TRANG_BI));
                                    return Task.CompletedTask;
								}
								...
								if (!itemTempalte.IsTypeBody() || itemTempalte.Type == 5)
                                {
                                    character.CharacterHandler.SendMessage(Service.DialogMessage(TextServer.gI().TRANG_BI_ERROR));
                                    return Task.CompletedTask;
                                }

								var daNgucTu = character.CharacterHandler.GetItemBagByIndex(arrIndexUi[1]);
                                if ((daNgucTu == null || daNgucTu.Id == 1112 || daNgucTu.Quantity < 10) &&
                                    (daNgucTu == null || daNgucTu.Id == 1113 || daNgucTu.Quantity < 10) &&
                                    (daNgucTu == null || daNgucTu.Id == 1114 || daNgucTu.Quantity < 10))
								{
									character.CharacterHandler.SendMessage(Service.DialogMessage(TextKhamDa.Gi().THIEU_NGUYEN_LIEU_DA_NGUC_TU));
                                    return Task.CompletedTask;
								}
								
								var kiemTraOption = itemBag.Options.FirstOrDefault(option => (option.Id == 233 || option.Id == 234 || option.Id == 235) && option.Param == 10000);
                                if (kiemTraOption != null)
						        {
							        character.CharacterHandler.SendMessage(Service.ServerMessage("Thất bại"));
									return Task.CompletedTask;
						        }
								
								var dataCombinne = KhamDa.CountKhamDa;
                                var checkDiamond = dataCombinne[0] < character.AllDiamond();
                                var checkGold = dataCombinne[1] < character.InfoChar.Gold;
								...
								itemBag.Options.ForEach(option =>
								{
									if (daNgucTu.Id == 1112)
									{
										if (option.Id != 233 && option.Id != 72)
										{
										    info +=$"{ServerUtils.Color("brown")}{ItemCache.ItemOptionTemplate(option.Id).Name.Replace("#", option.Param + "")}";
										}
                                        else if (option.Id == 233)
										{
											paramGetPlus = option.Param + 100;
									        info += $"{ServerUtils.Color("red")}Khảm đá: Tấn công + {paramGetPlus}";
										}
										
									}
									else if (daNgucTu.Id == 1113)
									{
										if (option.Id != 234 && option.Id != 72)
										{
										    info +=$"{ServerUtils.Color("brown")}{ItemCache.ItemOptionTemplate(option.Id).Name.Replace("#", option.Param + "")}";
										}
                                        else if (option.Id == 234)
										{
											paramGetPlus = option.Param + 100;
									        info += $"{ServerUtils.Color("red")}Khảm đá: Hp + {paramGetPlus}";
										}											
									}
									else if (daNgucTu.Id == 1114)
									{
										if (option.Id != 235 && option.Id != 72)
										{
										    info +=$"{ServerUtils.Color("brown")}{ItemCache.ItemOptionTemplate(option.Id).Name.Replace("#", option.Param + "")}";
										}
                                        else if (option.Id == 235)
										{
											paramGetPlus = option.Param + 100;
									        info += $"{ServerUtils.Color("red")}Khảm đá: Hp + {paramGetPlus}";
										}											
								...
								info += "\n";
								info += $"{ServerUtils.Color("red")}Chỉ số tối đa khi khảm đá: 100000";
								/*info += $"{ServerUtils.Color(checkGold ? "blue" : "red")}{string.Format(TextServer.gI().NEED_GOLD, ServerUtils.GetMoney(dataCombinne[1]))}";
                                info += $"{ServerUtils.Color(checkDiamond ? "blue" : "red")}{string.Format(TextServer.gI().NEED_DIAMOND, dataCombinne[0])}";*/
								
								character.CharacterHandler.SendMessage(Service.OpenUiConfirm(71, info, new List<string>() { "Nâng cấp\n" + ServerUtils.GetMoney(dataCombinne[1]) + " vàng\n" + dataCombinne[0] + " ngọc", "Từ chối" },character.InfoChar.Gender));
							...
							#region Ghép đá ngục tù
                            case 12:
                                {
                                    if (arrIndexUi.Count != 1)
                                    {
                                        character.CharacterHandler.SendMessage(
                                            Service.DialogMessage(TextKhamDa.Gi().THIEU_NGUYEN_LIEU_MANH_DA_NGUC_TU));
                                        return Task.CompletedTask;
                                    }
                                        ...
                                        character.CharacterHandler.GetItemBagByIndex(i)))
                                    {
                                        ...
                                        if (itemBag.Id is 1115 &&
                                            (itemBag.Id != 1115 || itemBag.Quantity >= 5)) continue;
                                        character.CharacterHandler.SendMessage(
                                            Service.DialogMessage(TextKhamDa.Gi().THIEU_NGUYEN_LIEU_MANH_DA_NGUC_TU));
                                        return Task.CompletedTask;
                                    }
                                    ...
                                    character.CharacterHandler.SendMessage(
                                        Service
                                            .OpenUiConfirm(71, TextKhamDa.Gi().TextKhamDaTrangBi[1], TextKhamDa.Gi().MenuKhamDaTrangBi[3],
                                                character.InfoChar.Gender));
							...
							#region Nâng cấp skh
							case 13: // đổi skh từ 2 món thần linh
                                {
                                    if (arrIndexUi.Count != 2)
                                    {
                                        character.CharacterHandler.SendMessage(Service.DialogMessage("Yêu cầu 2 trang bị Thần linh"));
                                        return Task.CompletedTask;
                                    }
                                    if (character.InfoChar.Gold < 100000000)
                                    {
                                        character.CharacterHandler.SendMessage(
                                                Service.DialogMessage("Yêu cầu phải có 100tr vàng"));
                                        return Task.CompletedTask;
                                    }
                                    var countTb = 0;
                                    ...
                                    var info = $"{ServerUtils.Color("black")}Trang bị được nâng cấp '" + ItemCache.ItemTemplate(character.CharacterHandler.GetItemBagByIndex(arrIndexUi[0]).Id).Name + "'";
                                    info += $"{ServerUtils.Color("brown")}Ngẫu nhiên 1 trong 3 set kích hoạt";
                                    info += $"{ServerUtils.Color("blue")}Tỷ lệ thành công: 100%";
                                    info += $"{ServerUtils.Color("blue")}Cần 100tr vàng";
                                    character.CharacterHandler.SendMessage(
                                        Service
                                            .OpenUiConfirm(80, info,
                                                new List<string>() { "Đổi", "Từ chối" },
                                                character.InfoChar.Gender));
                                    character.TypeMenu = 2;
                                    character.CombinneIndex = new List<int>()
                                {
                                ...
                                break;
                            case 14: // đổi skh từ 3 món skh khác
                                {
                                    if (arrIndexUi.Count != 3)
                                    {
                                        character.CharacterHandler.SendMessage(
                                                Service.DialogMessage("Yêu cầu 3 trang bị SKH"));
                                        return Task.CompletedTask;
                                    }
                                    if (character.InfoChar.Gold < 100000000)
                                    {
									...
									//check xem có phải là trang bị skh k
                                    var countTb = 0;
                                    for (int i = 0; i < arrIndexUi.Count; i++)
                                    {
                                        var item = character.CharacterHandler.GetItemBagByIndex(arrIndexUi[i]);
										...
										{
											if((option.Id == 127) || (option.Id == 128) || (option.Id == 129) ||
											   (option.Id == 130) || (option.Id == 131) || (option.Id == 132) ||
											   (option.Id == 136) || (option.Id == 137) || (option.Id == 138) ||
											   (option.Id == 139) || (option.Id == 140) || (option.Id == 141) ||
											   (option.Id == 142) || (option.Id == 143) || (option.Id == 144))
											{
												countTb++;
											}
										});
									...
									var countTb1 = 0;
                                    for (int y = 0; y < arrIndexUi.Count; y++)
                                    {
                                        var item = character.CharacterHandler.GetItemBagByIndex(arrIndexUi[y]);
                                        if ((item.Id >= 0 && item.Id <= 2) || (item.Id >= 6 && item.Id <= 8) ||
										    (item.Id >= 21 && item.Id <= 23) || (item.Id >= 27 && item.Id <= 29) || item.Id == 12 )
                                        {
                                            countTb1++;
                                        }
                                    }
                                    if (countTb != 3 && countTb1 != 3)
                                    {
                                        character.CharacterHandler.SendMessage(Service.DialogMessage("Yêu cầu 3 trang bị SKH"));
                                        return Task.CompletedTask;
                                    }
									
                                    var info = $"{ServerUtils.Color("black")}Trang bị SKH mới '" + ItemCache.ItemTemplate(character.CharacterHandler.GetItemBagByIndex(arrIndexUi[0]).Id).Name + "'";
                                    info += $"{ServerUtils.Color("brown")}Ngẫu nhiên 1 trong 3 set kích hoạt";
                                    info += $"{ServerUtils.Color("blue")}Tỷ lệ thành công: 100%";
                            ...
                            case 15: //1 món skh+ 3 thần linh 
                                {
                                    if (arrIndexUi.Count != 4)
                                    {
                                        character.CharacterHandler.SendMessage(Service.DialogMessage("Thiếu trang bị"));
                                        return Task.CompletedTask;
                                    }
                                    ...
                                    if (countTb3 != 1 && countTb4 != 1)
                                    {
                                        character.CharacterHandler.SendMessage(Service.DialogMessage("Yêu cầu 1 trang bị SKH"));
                                        return Task.CompletedTask;
                                    }
                                    ...
                                    {
                                        character.CharacterHandler.SendMessage(
                                                Service.DialogMessage("Yêu cầu 3 trang bị thần linh"));
                                        return Task.CompletedTask;
                                    }
                                    if (character.InfoChar.Gold < 100000000)
                                    {
